...
func convertToSecondLevelRevoke(bo *breachedOutput, breachInfo *retributionInfo,
	...
	bo.amt = qtumsuite.Amount(newAmt)
...
justiceTxBroadcast:
	...
	case _, ok := <-confChan.Confirmed:
		...
		var totalFunds, revokedFunds qtumsuite.Amount
...
func (bo *breachedOutput) Amount() qtumsuite.Amount {
	return bo.amt
}
...
func (b *breachArbiter) sweepSpendableOutputsTxn(txWeight int64,
	...
	var totalAmt qtumsuite.Amount
	for _, input := range inputs {
		totalAmt += qtumsuite.Amount(input.SignDesc().Output.Value)
	}
...
func (bo *breachedOutput) Decode(r io.Reader) error {
	...
	bo.amt = qtumsuite.Amount(binary.BigEndian.Uint64(scratch[:8]))
