...
func CreateTestChannels(tweaklessCommits bool) (
	*LightningChannel, *LightningChannel, func(), error) {
	...
	channelCapacity, err := qtumsuite.NewAmount(10)
	if err != nil {
		return nil, nil, nil, err
	}

	channelBal := channelCapacity / 2
	aliceDustLimit := qtumsuite.Amount(200)
	bobDustLimit := qtumsuite.Amount(1300)
	...
	aliceCommit := channeldb.ChannelCommitment{
		...
		CommitFee:     commitFee,
		FeePerKw:      qtumsuite.Amount(feePerKw),
...
func blockFromHex(blockHex string) (*qtumsuite.Block, error) {
	bytes, err := hex.DecodeString(blockHex)
	if err != nil {
		return nil, err
	}
	return qtumsuite.NewBlockFromBytes(bytes)
}

// txFromHex parses a full Bitcoin transaction from a hex encoded string.
func txFromHex(txHex string) (*qtumsuite.Tx, error) {
	bytes, err := hex.DecodeString(txHex)
	if err != nil {
		return nil, err
	}
	return qtumsuite.NewTxFromBytes(bytes)
}
...
func calcStaticFee(numHTLCs int) qtumsuite.Amount {
	const (
		...
		htlcWeight   = 172
		feePerKw     = qtumsuite.Amount(24/4) * 1000
	)
