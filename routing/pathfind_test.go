...
func parseTestGraph(path string) (*testGraphInstance, error) {
	...
	for _, edge := range g.Edges {
		...
		edgeInfo := channeldb.ChannelEdgeInfo{
			...
			Capacity:     qtumsuite.Amount(edge.Capacity),
		}
...
func symmetricTestChannel(alias1 string, alias2 string, capacity qtumsuite.Amount,
...
func TestPathInsufficientCapacity(t *testing.T) {
	...
	payAmt := lnwire.NewMSatFromSatoshis(qtumsuite.SatoshiPerBitcoin)
	_, err = findPath(
		&graphParams{
			graph: graph.graph,
		},
