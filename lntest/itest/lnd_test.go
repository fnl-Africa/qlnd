...
import (
	...
	"github.com/qtumproject/qlnd/lnrpc/watchtowerrpc"
...
func calcStaticFee(numHTLCs int) qtumsuite.Amount {
	const (
		commitWeight = qtumsuite.Amount(724)
		htlcWeight   = 172
		feePerKw     = qtumsuite.Amount(50 * 1000 / 4)
	)
	return feePerKw * (commitWeight +
		qtumsuite.Amount(htlcWeight*numHTLCs)) / 1000
}
...
func createPayReqs(node *lntest.HarnessNode, paymentAmt qtumsuite.Amount,
...
func testOnchainFundRecovery(net *lntest.NetworkHarness, t *harnessTest) {
	...
	skipAndSend := func(nskip int) func(*lntest.HarnessNode) {
		return func(node *lntest.HarnessNode) {
			...
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			err = net.SendCoins(
				ctxt, qtumsuite.SatoshiPerBitcoin, node,
			)
			if err != nil {
	...
	// path.
	restoreCheckBalance(2*qtumsuite.SatoshiPerBitcoin, 2, 1, skipAndSend(9))

	// Check that using a recovery window of 9 does not find the two most
	// recent txns.
	restoreCheckBalance(2*qtumsuite.SatoshiPerBitcoin, 2, 9, nil)
	...
	restoreCheckBalance(4*qtumsuite.SatoshiPerBitcoin, 4, 10, skipAndSend(19))

	// Check that recovering with a recovery window of 19 fails to find the
	// most recent transactions.
	restoreCheckBalance(4*qtumsuite.SatoshiPerBitcoin, 4, 19, nil)

	// Ensure that using a recovery window of 20 succeeds with all UTXOs
	// found and the final balance reflected.
	restoreCheckBalance(6*qtumsuite.SatoshiPerBitcoin, 6, 20, nil)
}
...
func basicChannelFundingTest(t *harnessTest, net *lntest.NetworkHarness,
	...
	pushAmt := qtumsuite.Amount(100000)
...
test:
	...
	for _, carolTweakless := range []bool{true, false} {
		for _, daveTweakless := range []bool{true, false} {
			...
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			err = net.SendCoins(ctxt, qtumsuite.SatoshiPerBitcoin, carol)
			if err != nil {
...
func calculateMaxHtlc(chanCap qtumsuite.Amount) uint64 {
...
func testChannelBalance(net *lntest.NetworkHarness, t *harnessTest) {
	...
	checkChannelBalance := func(node lnrpc.LightningClient,
		...
		balance := qtumsuite.Amount(response.Balance)
		if balance != amount {
...
func testChannelUnsettledBalance(net *lntest.NetworkHarness, t *harnessTest) {
	const chanAmt = qtumsuite.Amount(1000000)
	ctxb := context.Background()
...
func testChannelForceClosure(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		chanAmt     = qtumsuite.Amount(10e6)
		pushAmt     = qtumsuite.Amount(5e6)
		paymentAmt  = 100000
		numInvoices = 6
	)
	...
	carolExpectedBalance := qtumsuite.Amount(carolStartingBalance) + pushAmt
	if qtumsuite.Amount(carolBalResp.ConfirmedBalance) < carolExpectedBalance {
		t.Fatalf("carol's balance is incorrect: expected %v got %v",
...
func testSphinxReplayPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	chanAmt := qtumsuite.Amount(100000)
	...
	assertAmountSent := func(amt qtumsuite.Amount) {
...
func testMultiHopPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = qtumsuite.Amount(100000)
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, qtumsuite.SatoshiPerBitcoin, dave)
	if err != nil {
...
func testMultiHopSendToRoute(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, qtumsuite.SatoshiPerBitcoin, net.Bob)
	if err != nil {
...
func testSendToRouteErrorPropagation(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, qtumsuite.SatoshiPerBitcoin, charlie)
	if err != nil {
...
func testInvoiceSubscriptions(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = qtumsuite.Amount(500000)
...
func testMaxPendingChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	carolBalance := qtumsuite.Amount(maxPendingChannels) * amount
...
func testRevokedCloseRetributionAltruistWatchtower(net *lntest.NetworkHarness,
	...
	// Before we make a channel, we'll load up Dave with some coins sent
	// directly from the miner.
	err = net.SendCoins(ctxb, qtumsuite.SatoshiPerBitcoin, dave)
	if err != nil {
		t.Fatalf("unable to send coins to dave: %v", err)
	}
...
func testRejectHTLC(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Send coins to Carol.
	err = net.SendCoins(ctxb, qtumsuite.SatoshiPerBitcoin, carol)
	if err != nil {
		t.Fatalf("unable to send coins to carol: %v", err)
	}

	// Send coins to Alice.
	err = net.SendCoins(ctxb, qtumsuite.SatoshiPerBitcent, net.Alice)
	if err != nil {
			...
			for _, chanUpdate := range graphUpdate.ChannelUpdates {
				...
				if chanUpdate.Capacity != int64(chanAmt) {
					t.Fatalf("channel capacities mismatch:"+
						...
						qtumsuite.Amount(chanUpdate.Capacity))
				}
				numChannelUpds++
			}
...
func testAsyncPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ := context.WithTimeout(ctxb, channelOpenTimeout)
	channelCapacity := qtumsuite.Amount(paymentAmt * 2000)
	chanPoint := openChannelAndAssert(
		ctxt, t, net, net.Alice, net.Bob,
		lntest.OpenChannelParams{
			Amt: channelCapacity,
	...
	availableBalance := qtumsuite.Amount(info.LocalBalance) - chanReserve
...
func testMultiHopHtlcLocalTimeout(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		dustHtlcAmt    = qtumsuite.Amount(100)
		htlcAmt        = qtumsuite.Amount(30000)
		finalCltvDelta = 40
	)
...
func testSwitchCircuitPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = qtumsuite.Amount(1000000)
	const pushAmt = qtumsuite.Amount(900000)
	var networkChans []*lnrpc.ChannelPoint
...
func testSendUpdateDisableChannel(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Give Eve some coins.
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, qtumsuite.SatoshiPerBitcoin, eve)
	if err != nil {
...
func testSweepAllCoins(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, qtumsuite.SatoshiPerBitcoin, ainz)
	if err != nil {
		t.Fatalf("unable to send coins to eve: %v", err)
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoinsNP2WKH(ctxt, qtumsuite.SatoshiPerBitcoin, ainz)
	if err != nil {
		t.Fatalf("unable to send coins to eve: %v", err)
	}
...
func testChannelBackupUpdates(net *lntest.NetworkHarness, t *harnessTest) {
	...
	numChans := 2
	chanAmt := qtumsuite.Amount(1000000)
...
func testChanRestoreScenario(t *harnessTest, net *lntest.NetworkHarness,
	...
	const (
		chanAmt = qtumsuite.Amount(10000000)
		pushAmt = qtumsuite.Amount(5000000)
	)
...
func TestLightningNetworkDaemon(t *testing.T) {
	...
	handlers := &rpcclient.NotificationHandlers{
		OnTxAccepted: func(hash *chainhash.Hash, amt qtumsuite.Amount) {
