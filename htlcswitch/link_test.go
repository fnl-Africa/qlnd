...
func TestChannelLinkSingleHopPayment(t *testing.T) {
	...
	alice, bob, cleanUp, err := createTwoClusterChannels(
		qtumsuite.SatoshiPerBitcoin*3,
		qtumsuite.SatoshiPerBitcoin*5)
	if err != nil {
	...
	amount := lnwire.NewMSatFromSatoshis(qtumsuite.SatoshiPerBitcoin)
...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	const amount = qtumsuite.SatoshiPerBitcoin
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	amountNoFee := lnwire.NewMSatFromSatoshis(qtumsuite.SatoshiPerBitcoin)
...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		qtumsuite.SatoshiPerBitcoin*5, qtumsuite.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// rejected.
	amount := lnwire.NewMSatFromSatoshis(4 * qtumsuite.SatoshiPerBitcoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func newSingleLinkTestHarness(chanAmt, chanReserve qtumsuite.Amount) (
...
func TestChannelLinkBandwidthConsistency(t *testing.T) {
	...
	const chanAmt = qtumsuite.SatoshiPerBitcoin * 5
...
func TestChannelLinkTrimCircuitsPending(t *testing.T) {
	...
	const (
		chanAmt   = qtumsuite.SatoshiPerBitcoin * 5
		numHtlcs  = 4
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	// channel reserve.
	const chanAmt = qtumsuite.SatoshiPerBitcoin * 5
	const chanReserve = qtumsuite.SatoshiPerBitcoin * 1
	...
	// a switch initiated payment.  The resulting bandwidth should
	// now be decremented to reflect the new HTLC.
	htlcAmt := lnwire.NewMSatFromSatoshis(3 * qtumsuite.SatoshiPerBitcoin)
	invoice, htlc, _, err := generatePayment(htlcAmt, htlcAmt, 5, mockBlob)
	if err != nil {
		t.Fatalf("unable to create payment: %v", err)
	}
	...
	const bobChanAmt = qtumsuite.SatoshiPerBitcoin * 1
	const bobChanReserve = qtumsuite.SatoshiPerBitcoin * 1.5
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	// for this test.
	const aliceInitialBalance = qtumsuite.SatoshiPerBitcoin * 3
	channels, cleanUp, _, err := createClusterChannels(
		aliceInitialBalance, qtumsuite.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
